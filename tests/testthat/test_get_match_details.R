...
test_that('function get_match_details works properly', {
 ...
 expect_error(get_match_details(language = 'en',
                                    key = NULL),
 ...
 expect_equal(get_match_details()$response$status_code,
              200L)

 #make sure user_agent has been set properly
 expect_identical(
  get_live_league_games()$response$request$options$useragent, 'http://github.com/lyzander/RDota2'
 )

 #make sure function returned a results list
 expect_true(is.list(get_match_details()$content))
