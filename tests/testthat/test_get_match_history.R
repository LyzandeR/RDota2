context("test get_match_history is working")
...
test_that('function get_match_history works properly', {
 ...
 expect_equal(get_match_history()$response$status_code,
              200L)

 #make sure user_agent has been set properly
 expect_identical(
  get_match_history()$response$request$options$useragent, 'http://github.com/lyzander/RDota2'
 )
